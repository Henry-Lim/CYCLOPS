...
class cyclops:
    def __init__(self, input_width, hidden_size=5):
        ...
        input_layer_drop = keras.layers.Dropout(0.2)(input_layer)

        """
        Hidden Layer
        """
        hidden_layer_in = keras.layers.Dense(name = 'hidden_input', units=hidden_size,
                            activation='linear',bias_initializer=keras.initializers.random_normal(stddev=0.5))(input_layer_drop)

        hidden_layer_in_drop = keras.layers.Dropout(0.2)(hidden_layer_in)
        ...
        a0 = keras.layers.Dense(name='encoder_circular_in_0',
                                 ...
                                 bias_initializer=keras.initializers.random_normal(stddev=0.25),
                                 )(hidden_layer_in_drop)
        ...
        hidden_layer_out = keras.layers.Dense(name = 'hidden_output', units=hidden_size, activation='linear',
                                              bias_initializer=keras.initializers.random_normal(stddev=0.5))(x)

        hidden_layer_out_drop = keras.layers.Dropout(0.2)(hidden_layer_out)
        ...
        y = keras.layers.Dense(name='output_layer',
                                   units=input_width,
                                   kernel_initializer=kernel_initializer,
                                   bias_initializer='zeros')(hidden_layer_out_drop)

        y_drop = keras.layers.Dropout(0.2)(y)

        self.model = keras.Model(outputs=y_drop, inputs=input_layer)
    ...
    def train(self, data, batch_size=10, epochs=100, verbose=10, rate=0.3, momentum=0.5):
        ...
        self.model.compile(loss='cosine_similarity',
                           optimizer=opt)
    ...
    def predict_input(self, data):
        res = keras.backend.function(inputs=[self.model.get_layer('input_layer').input],
                                    outputs=[self.model.get_layer('output_layer').output])([data])
        return res
    ...
    def phase_output(self, data):
        ...
        """
        phase = self.predict_pseudotime(data)
        
        df_phase = pd.DataFrame(phase, columns=['pseudotime'])
